...
uint8_t isBlackJack(uint8_t& hand_sum, uint8_t& turn, uint8_t& is_blackjack){
  ...
  if(hand_sum == 21 && turn == 0){
      ...
      is_blackjack = 3;
  }else if(hand_sum == 21 && turn == 1){
      is_blackjack = 2;
  }
  return is_blackjack;
}

 void point_Sum(const vector<uint8_t> hand , uint8_t& point){
  point = 0;
  int k = 0;
  for(int i=0; i < hand.size(); i++){
    auto card_id = hand[i];
    auto card = card_dict.at(card_id);

      if(card.type == ACE){
          k += 1;
      }else{
      point += card.attack_point;
      }
  }
  for(int i=0; i < k; i++){
    if(point + 11 < 22){
...
int judge(uint8_t player_point, uint8_t ai_point, uint8_t is_blackjack){
  if(is_blackjack == 1){//BJかち
    ...
    return 1;
  } else if(player_point < 22 && ai_point > 21){
    return 3;
...
void transfer(account_name username, uint8_t amount, account_name to){
  ...
  public:
    ...
    cardgame( account_name self ):eosio::contract(self),_users(self, self),_seed(self, self){}
    ...
    // Draw 2 cards each for the player 
        for (int i = 0; i < 2; i++) {
            ...
            point_Sum(game_data.hand_player,game_data.point_player);
        }
        // Draw 1 cards(open) each for AI
        draw_one_card(game_data.deck, game_data.hand_ai,game_data.point_ai);
        point_Sum(game_data.hand_ai, game_data.point_ai);
        game_data.is_blackjack = isBlackJack(game_data.point_player,game_data.turn, modified_user.game_data.is_blackjack);
    ...
    void playcard(account_name username) {
        ...
        _users.modify(user, username, [&](auto& modified_user) {
          eosio_assert(modified_user.game_data.status == ONGOING,
               "playcard: This game has ended. Please start a new one");
          eosio_assert(modified_user.game_data.turn == 0, "It's not your turn!");
...
//one more card that was hidden
        draw_one_card(modified_user.game_data.deck, modified_user.game_data.hand_ai,modified_user.game_data.point_ai);
        point_Sum(modified_user.game_data.hand_ai, modified_user.game_data.point_ai);
        modified_user.game_data.is_blackjack = isBlackJack(modified_user.game_data.point_ai,modified_user.game_data.turn, modified_user.game_data.is_blackjack);
//dealer move
        if(modified_user.game_data.is_blackjack == 0 || modified_user.game_data.is_blackjack == 1){
          while (modified_user.game_data.point_ai < 17){// if point < 17, dealer has to draw.
            draw_one_card(modified_user.game_data.deck, modified_user.game_data.hand_ai, modified_user.game_data.point_ai);
            point_Sum(modified_user.game_data.hand_ai, modified_user.game_data.point_ai);
            }

          if(modified_user.game_data.point_player > modified_user.game_data.point_ai){//if player > dealer draw card
            draw_one_card(modified_user.game_data.deck, modified_user.game_data.hand_ai, modified_user.game_data.point_ai);
            point_Sum(modified_user.game_data.hand_ai, modified_user.game_data.point_ai);
          }
        }
        ...
        int win = judge(modified_user.game_data.point_player, modified_user.game_data.point_ai, modified_user.game_data.is_blackjack);
        if(win ==  1){//引き分け
            //statusを入れる
    ...
    void addtip(account_name ownername, account_name username){
    ...
    void refundbyowner(account_name ownername, account_name username){
        ...
        modified_user.refund_tip = 0;
         modified_user.refunding = 0;
        });
    }
/*
...
EOSIO_ABI(cardgame, (login)(startgame)(playcard)(changeturn)(endgame)(addtip))
